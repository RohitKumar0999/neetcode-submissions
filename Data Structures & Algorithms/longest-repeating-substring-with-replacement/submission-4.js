class Solution {
    ...
    /**
     * Approach: Variable sliding window + map
     * here we are shrinking window on condition not match and updating our stored freq map for each char
     * you can notice that after decreasing value in map we are not updating the maxFreq which make stale but bcz it does'nt impact\
     * our answer res as it will not exceed the val it has when it value is accurate
     * T.C: O(N);
     * S.C: O(M);
     * Where M is total unique chars and M is total chars
     * @param {string} s
     * @param {number} k
     * @return {number}
     */
